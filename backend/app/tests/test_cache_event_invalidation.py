...
@pytest.mark.asyncio
async def test_order_card_is_fresh_after_event_invalidation(db_session, monkeypatch):
    """
    ...
    try:
        ...
        await db_session.execute(
            ...
            {"id": user_id, "email": "cache-fresh@example.com", "name": "Fresh User"},
        )
        await db_session.execute(
            text(
                """
                INSERT INTO orders (id, user_id, status, total_amount, created_at)
                VALUES (:id, :user_id, 'created', 150, CURRENT_TIMESTAMP)
                """
            ),
        ...
        await db_session.execute(
            text(
                ...
                VALUES (:id, :order_id, 'Laptop', 150, 1, 150)
                """
            ),
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            assert fresh_response.json()["total_amount"] == 321.0
    finally:
